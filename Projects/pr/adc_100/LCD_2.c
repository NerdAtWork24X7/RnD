...
{   
   output_b(komut>>4);   // The higher value of the command sent the 4-bit
   ...
   output_b(komut&0x0F); // The command sends a 4-bit low -valued
...
{ 
   output_b(veri>>4);    // The higher value of the 4-bit data sent
   ...
   output_b(veri&0x0F); // The low 4 bits of valuable data sent
   output_high(rs);      // LCD veri almak için ayarlandý
   delay_cycles(1);      // 1 komut saykýlý bekle    
   output_high(e);      // E ucu lojik-1'den lojik-0'a çekiliyor
   delay_cycles(1);     // 1 komut saykýlý bekle
